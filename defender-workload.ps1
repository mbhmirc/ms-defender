...
$projectExtensions = @('.dll', '.pdb', '.json', '.config', '.log', '.cache', '.cs', '.csproj', '.targets', '.props', '.tmp')
...
while ($stopwatch.Elapsed.TotalSeconds -lt ($DurationSeconds * 0.85)) {
    $null = Start-Process -FilePath "cmd.exe" -ArgumentList "/c echo workload_$procCount > nul" -WindowStyle Hidden -PassThru
    $procCount++

    if ($procCount % 3 -eq 0) {
        $childObjDir = Join-Path $WorkDir 'project\obj\Debug\net8.0'
        $childCacheDir = Join-Path $WorkDir 'cache\nuget\packages'
        $miniScript = @"
`$objDir = '$($childObjDir -replace "'", "''")'
`$cacheDir = '$($childCacheDir -replace "'", "''")'
1..4 | ForEach-Object {
    `$dllPath = Join-Path `$objDir ('module_${procCount}_' + `$_ + '.dll')
    `$jsonPath = Join-Path `$cacheDir ('restore_${procCount}_' + `$_ + '.json')
    Set-Content -Path `$dllPath -Value ('x' * 2048)
    Add-Content -Path `$jsonPath -Value ('{""iteration"":' + `$_ + '}')
    Get-Content -Path `$dllPath -TotalCount 1 | Out-Null
}
"@
        $null = Start-Process -FilePath "powershell.exe" -ArgumentList @('-NoProfile', '-Command', $miniScript) -WindowStyle Hidden -PassThru
        $fileCount += 8
    }
